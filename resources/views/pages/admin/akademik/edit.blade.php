...
@section('content')
    ...
    <div class="card card-primary">
        ...
        <div class="card-body">
            <form action="{{route('tahun.akademik.admin.update', ['id' => $akademik->id])}}" method="POST">
                @csrf
                @method("PUT")
                <div class="form-group mb-3">
                    <label for="" class="mb-2">Tahun</label>
                    <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{$akademik->name}}">
                    @error('name')
